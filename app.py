...
def load_all_data():
    ...
    for nome_aba, gid in ABAS_CONFIG.items():
        try:
            ...
            df_temp = pd.read_csv(url, dtype=str) # Leitura inicial como texto puro para evitar distorções do CSV
...
if not df_raw.empty:
    ...
    # Mapeamento flexível para a coluna de Meta
    ...
    # Trata colunas numéricas sem alterar os valores
    ...
    for c in colunas_numericas:
        if c in df_raw.columns:
            # Limpa apenas espaços e converte com segurança para número inteiro/float
    ...
    # Soma do realizado real da linha (fevereiro até dezembro se existirem)
    ...
    with c_bar:
        ...
        resumo = df_resumo_base.groupby(COL_CLINICA, as_index=False).agg(agg_dict).rename(columns={'TOTAL_REALIZADO_LINHA': 'REALIZADO'})
